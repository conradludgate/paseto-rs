...
use super::{LocalKey, PublicKey, SecretKey};
...
pub(super) fn seal_key(sealing_key: &PublicKey, key: LocalKey) -> Result<Box<[u8]>, PasetoError> {
    ...
    let xk = curve25519::scalarmult(esk.as_bytes(), &xpk).map_err(|_| PasetoError::CryptoError)?;
    ...
    n.update(&xpk);
    let n =
        xchacha20::Nonce::try_from_slice(&n.finalize()).map_err(|_| PasetoError::CryptoError)?;
